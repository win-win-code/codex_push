...
Уведомления о событиях отправляются только когда CoreGraphics подтверждает, что
экран macOS заблокирован. При разблокированном экране или если состояние
определить не удалось, событие молча пропускается. Команда `--test` не зависит
от блокировки, чтобы её можно было проверить сразу после установки.
