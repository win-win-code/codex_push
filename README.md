...
Уведомления о событиях не отправляются, когда CoreGraphics подтверждает, что
экран macOS разблокирован. Уведомление отправляется при заблокированном или
неопределённом состоянии: Codex запускает hook вне GUI-сессии, где CoreGraphics
корректно может не вернуть её статус. Команда `--test` не зависит от блокировки,
чтобы её можно было проверить сразу после установки.
